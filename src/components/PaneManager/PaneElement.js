...
class PaneElement extends React.Component {
  ...
  onMinimizeClicked() {
    ...
    const update = size => {
      return new Promise(resolve => {
        const newSize = size < this.props.minSize ? this.props.minSize : size;
    ...
    this.animate(currentSize, this.props.minSize, -450, done, update).then(
      () => {
  ...
  onMaximizeClicked() {
    ...
    this.animate(currentSize, window.innerHeight, 450, done, update).then(
      () => {
        if (this.props.onStopResize) {
          this.props.onStopResize();
        }
  ...
  renderControls() {
    ...
    return (
      <div className="controls">
        ...
        <button onClick={this.onMinimizeClicked} title="minimize widget">
