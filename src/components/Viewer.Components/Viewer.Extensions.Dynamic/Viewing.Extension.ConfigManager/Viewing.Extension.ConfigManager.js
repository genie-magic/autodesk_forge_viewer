...
class ConfigManagerExtension extends MultiModelExtensionBase {
    ...
    this.react
      .setState({
        emptyStateNameCaption:
          this.options.emptyStateNameCaption || 'State Name ...',
  ...
  async loadSequences() {
    ...
    const sequence = sequences.length ? sequences[0] : null;
  ...
  /////////////////////////////////////////////////////////
  setModel(model) {
    const modelId = model.dbModelId || this.options.dbModel._id;

    const database = model.database || this.options.database;

    const { apiUrl } = this.options;

    this.api = new ConfigAPI(`${apiUrl}/config/${database}/${modelId}`);
  }
  ...
  addSequence() {
    ...
    const onClose = result => {
      if (result === 'OK') {
        ...
        const sequences = sortBy([...state.sequences, sequence], s => {
          return s.name;
        });
  ...
  copySequence() {
    ...
    const onClose = result => {
      if (result === 'OK') {
        ...
        const sequence = Object.assign({}, state.sequence, {
  ...
  deleteSequence() {
    ...
    const onClose = result => {
      if (result === 'OK') {
        ...
        const sequences = state.sequences.filter(sequence => {
    ...
    const msg = DOMPurify.sanitize(
      `Are you sure you want to delete` + ` <b>${state.sequence.name}</b> ?`
    );
  ...
  /////////////////////////////////////////////////////////
  getStateIds() {
    const domItems = document.getElementsByClassName(this.itemsClass)[0];

    const stateIds = Array.from(domItems.childNodes).map(childNode => {
  ...
  async onUpdateSequence() {
    ...
    const newSequence = Object.assign({}, sequence, {
      stateIds
    });
  ...
  async addItem() {
    ...
    const viewerState = Object.assign({}, this.viewer.getState(), {
      id: this.guid(),
      name
    });

    if (this.api) {
      this.api.addState(state.sequence.id, viewerState);
    }
  ...
  onRestoreState(viewerState, immediate = this.options.restoreImmediate) {
    ...
    this.viewer.restoreState(filteredState, this.restoreFilter, immediate);
  }
  ...
  async setActiveSequence(sequence) {
    ...
    if (sequence) {
      ...
      if (this.api) {
        const states = await this.api.getStates(sequence.id);
  ...
  renderControls() {
    ...
    const sequenceName = sequence
      ? sequence.name + (!sequence.readonly ? '' : ' (readonly)')
      : '';

    const stateCreationDisabled =
      !sequence || sequence.readonly || !state.stateCreation;
    ...
    return (
      <div className="controls">
        <div className="row">
          ...
          <button onClick={() => this.addSequence()} title="Add sequence">
  ...
  renderItems() {
    ...
    return <div className={`items ${this.itemsClass}`}>{items}</div>;
  }
  ...
  render(opts) {
    ...
    return (
      ...
      >
        {state.disabled && <div className="disabled-overlay" />}
        {this.renderControls()}
        {this.renderItems()}
      </WidgetContainer>
    );
