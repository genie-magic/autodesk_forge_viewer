...
import './Viewing.Extension.ModelLoader.css';
...
class ModelLoaderExtension extends MultiModelExtensionBase {
  ...
  load() {
    ...
    const activeModel = models.length ? models[0] : null;
    ...
    this.viewer
      ...
      .then(ctxMenuExtension => {
        ctxMenuExtension.addHandler(this.onContextMenu);
      });
  ...
  showModelDlg() {
    ...
    if (this.options.database === 'gallery_dump') {
      const db_gallery_dumb = [
        {
          ...
          model: {
            ...
            urn:
              'dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6Zm9yZ2UtcmNkYi1nYWxsZXJ5LXRtcC1wcm9kL2EyODMtNmZkZC0yYzU1LmYzZA'
          }
        },
        {
      ...
      const dbModelsByName = sortBy(db_gallery_dumb, model => {
        return model.name;
      });
      ...
      this.modelSvc.getModels(this.options.database).then(models => {
        const dbModelsByName = sortBy(models, model => {
          return model.name;
        });
  ...
  loadModel(dbModel) {
    return new Promise(async resolve => {
      ...
      const fileType = dbModel.fileType || this.getFileType(dbModel.model.urn);

      const loadOptions = {
        placementTransform: this.buildPlacementTransform(fileType)
      };

      switch (dbModel.env) {
        case 'AutodeskProduction':
          const lmvProxy = dbModel.model.proxy || 'lmv-proxy-2legged';
          ...
          const doc = await Toolkit.loadDocument(dbModel.model.urn);
          ...
          if (items.length) {
            ...
            this.viewer.loadModel(path, loadOptions, model => {
              model.database = dbModel.database || this.options.database;
  ...
  async unloadModel() {
    ...
    const msg = DOMPurify.sanitize(
      `Are you sure you want to unload` + `<b><br/>${activeModel.name}</b> ?`
    );
        ...
        content: (
          <div>
            ...
            <ContentEditable
              ...
              className="search"
            />
            <div className="scroller">{modelDlgItems}</div>
          </div>
        )
      },
      true
  ...
  renderControls() {
    ...
    const modelName = activeModel ? activeModel.name : '';
    ...
    return (
      <div className="controls">
        <div className="row">
          ...
          <button onClick={() => this.showModelDlg()} title="Load model">
